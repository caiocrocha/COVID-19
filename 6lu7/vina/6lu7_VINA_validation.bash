...
if [ ! -f local.pdbqt ] ; then 
   vina --receptor ../receptor.pdbqt --ligand crystal.pdbqt --out local.pdbqt  --log local.log    ${CENTER_SIZE} --local_only
fi

if [ ! -f random.pdbqt ] ; then 
   vina --receptor ../receptor.pdbqt --ligand crystal.pdbqt --out random.pdbqt --log random.pdbqt ${CENTER_SIZE} --randomize_only
fi
    ...
    for run in 1 2 3 ; do
        ...
        for ex in "8" "24" "32" ; do 
            ...
            for er in "1" "3" ; do
              ...
              # Run docking
         echo "vina --receptor ../receptor.pdbqt \
                      --ligand ${ligand}.pdbqt \
                      --out ${name}.pdbqt  \
                      --log ${name}.log \
                      ${CENTER_SIZE} \
